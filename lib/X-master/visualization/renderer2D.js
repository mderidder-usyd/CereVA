...
X.renderer2D = function() {
  ...
  /**
   * Michael: This flag is set so new lines can be drawn in onRender when changes occur.
   *
   * @type {!boolean}
   * @protected
   */
...
X.renderer2D.prototype.render_ = function(picking, invoked) {
  ...
  if (_redraw_required){
    //Michael - set the flag for onRender
    this._redraw = true;
    //End Michael

    //Michael - moved from outside if statement
    ...
    //Michael inner (13/11/15)
    //var currVolume, currVolumeSlice;
    var extraVolumesSliceData = new Array();
    for (var objNum=1; objNum<_numberOfObjects; objNum++){
      currVolume = this._topLevelObjects[objNum];
      if (this._orientationIndex == 0) {
        currVolumeSliceData = currVolume._slicesX._children[_currentSlice]._texture._rawData;
      } 
      else if (this._orientationIndex == 1) {
        currVolumeSliceData = currVolume._slicesY._children[_currentSlice]._texture._rawData;
      } 
      else {
        currVolumeSliceData = currVolume._slicesZ._children[_currentSlice]._texture._rawData;
      }

      extraVolumesSliceData.push(currVolumeSliceData);
    }
    //End Michael inner
    ...
    //Michael
    if(extraVolumesSliceData.length > 0){
      //Update _imageData for other slices
      for (var ev=0; ev<extraVolumesSliceData.length; ev++){
        console.log("extraVolumesSliceData " + ev);
        console.log(extraVolumesSliceData);
        //_pixels = _imageData.data;
        /*for (var pix=0; pix<extraVolumesSliceData[ev].length; pix+=4){
          if (extraVolumesSliceData[ev][pix] != 0 || extraVolumesSliceData[ev][pix+1] != 0 || extraVolumesSliceData[ev][pix+2] != 0){ //Don't check opacity
            //TEMP: make blue
            _pixels[pix] = 0;
            _pixels[pix+1] = 0;
            _pixels[pix+2] = 255;
    ...
    //Michael - Change the colour of pixels for barometric lines
    _pixels = _imageData.data;
    for (var pix=0; pix<_pixels.length; pix+=4){
      if (_pixels[pix] < 0 || _pixels[pix+1] < 0 || _pixels[pix+2] < 0){ //Don't check opacity
        console.log(pix + ":" + _pixels[pix] + "; " + pix+1 + ":" + _pixels[pix+1] + "; " + pix+2 + ":" + _pixels[pix+2]);
        /*_pixels[pix] = 0;
        _pixels[pix+1] = 0;
        _pixels[pix+2] = 255;*/
      }
  ...
  // draw the slice (Michael added if statement for redraw)
  if (_redraw_required){
